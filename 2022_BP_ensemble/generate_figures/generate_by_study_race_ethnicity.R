...
colnames(pheno_file)[which(colnames(pheno_file) == "study")] <- c("Study")

trial2 <- pheno_file %>% select(Study,"Race/Ethnicity")
...
table2 %>%
  as_flex_table() %>%
  flextable::save_as_docx(path = "~/Dropbox (Partners HealthCare)/2022_BP_ensemble/Figures/TOPMed/Study_by_Race_Ethnicity_portrait.docx", pr_section = sect_properties)
  #flextable::save_as_image(path = "~/Dropbox (Partners HealthCare)/2022_BP_ensemble/Figures/TOPMed/Study_by_Race_Ethnicity_portrait.png", webshot = "webshot")
